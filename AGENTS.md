...
- 날짜별 작업 요약과 검증 기록은 `docs/worklog/YYYY-MM-DD.md`에 남기고, [docs/WORKLOG.md](docs/WORKLOG.md)에는 인덱스를 갱신한다.
...
하네스의 세부 흐름도는 [Verification Harness](docs/12-verification-harness.md)를 따른다.
...
사용자가 “하루작업 정리해줘”, “오늘 작업 정리해줘”, “작업 일지 정리해줘”처럼 요청하면 `docs/worklog/YYYY-MM-DD.md`에 오늘 날짜 기준 작업 기록을 추가하거나 갱신한다. 날짜가 바뀌면 새 파일을 만든다.

[docs/WORKLOG.md](docs/WORKLOG.md)는 날짜별 worklog 파일로 이동하는 인덱스와 과거 기록 보관 용도로 사용한다.
...
날짜별 worklog는 진행 상황을 사람이 읽는 일지이고, `TODO.md`는 체크리스트, `LESSONS.md`는 배운 점 기록이다. 세 문서의 역할을 섞지 않는다.
