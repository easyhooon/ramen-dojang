...
이번 214건 catalog seed는 `server/api/src/main/resources/seed/shops.seed.json`에 둔다. GitHub에 올려도 되는 공개 후보 데이터지만, 운영에서 검수 완료된 canonical 데이터가 아니라 초기 seed 후보/검수 전 데이터라는 성격을 유지한다. export 후에는 JSON parse, row count, `id` 필드 미포함 여부를 확인한다. 아직 자동 import는 만들지 않고, 필요해질 때 작은 script나 admin endpoint로 읽어 넣는다.
