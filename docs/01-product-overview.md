...
초기 목표는 앱스토어/플레이스토어 배포용 native wrapper나 토스 로그인 있는 개인화 서비스가 아니라, 같은 웹 코어를 토스 미니앱으로 먼저 올릴 수 있는지 검증하는 것이다. 1차 샌드박스 확인은 공용 라멘집 카탈로그를 서버에서 받고 개인 방문 기록과 위시리스트는 로컬 저장에 둔다. 다만 개인 기록을 서버에 저장하는 단계로 넘어가면 최소한 앱인토스 `getAnonymousKey` 기반 익명 사용자 식별을 붙인다.
...
- 한국어를 기본으로 시작하되, 한국 밖의 사용자도 고려해 영어와 일본어를 지원할 수 있게 확장한다.
...
- 기본 About 페이지
- 설정에서 언어 선택: 기본은 시스템 언어를 따르고, 사용자가 한국어/영어/일본어 중 하나로 고정할 수 있게 한다.
...
첫 앱인토스 샌드박스 확인은 토스 로그인 없이 낸다. 다만 라멘집 검색 데이터는 앱에 하드코딩하지 않고 서버 catalog API에서 받는다. 개인 방문 기록과 위시리스트를 서버에 저장하려면 로컬 저장만으로는 부족하므로, 토스 로그인보다 가벼운 `getAnonymousKey` 기반 익명 식별을 먼저 붙인다. 토스 로그인은 실제 개인정보 조회나 결제 상태 조회가 필요해질 때 재검토한다.
