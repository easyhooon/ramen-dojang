...
초기 목표는 앱스토어/플레이스토어 배포용 native wrapper나 로그인 있는 서버 서비스가 아니라, 같은 웹 코어를 토스 미니앱으로 먼저 올릴 수 있는지 검증하는 것이다. 1차 MVP는 로그인 없이 로컬 저장만으로 다녀온 라멘집, 가고 싶은 라멘집, 방문 기록을 다루는 아기자기한 개인 기록 앱으로 둔다. 서버, 계정, 지도는 기록 경험이 쓸 만해진 뒤 붙인다.
...
1차 MVP는 지도 기능이나 서버보다 로컬 기록 경험과 앱인토스 샌드박스 실행을 우선한다.

### 포함

- 로컬 저장 기반 라멘집 등록, 목록, 상세, 수정, 삭제
...
3. 라멘집, 방문 기록, 위시리스트를 `localStorage` 저장 모드로 전환
4. 모바일 우선 화면으로 홈, 목록, 상세, 작성/수정 흐름 정리
5. TDS 컴포넌트 적용 범위 산정
6. 앱 정보와 약관 등록
7. `.ait` 업로드 후 샌드박스에서 서버 없이 수동 테스트
8. 사용감 확인 뒤 서버 동기화와 로그인 필요성 재판단

## 9. 작업 시작 순서

```text
1. 현재 프론트 화면의 API 의존 지점 확인
2. localStorage local repository 작성
3. 라멘집/방문 기록/위시리스트 화면을 local repository로 연결
4. 모바일 폭에서 화면 깨짐 확인
5. build:ait 재실행
...
첫 앱인토스 MVP는 로그인과 서버 없이 로컬 저장 기반 개인 기록 앱으로 낸다. 서버/API/DB/PostGIS는 삭제하지 않고, 기기 간 동기화, 공개 라멘집 DB, 장소 후보 수집, 토스 로그인 같은 요구가 실제로 필요해질 때 2차 경로로 재개한다.

### Local record first, map and server later

지도는 서비스의 중요한 정체성이지만 1차 MVP의 핵심은 아니다. 먼저 라멘집과 방문 기록을 로컬 저장으로 가볍게 생성, 조회, 수정, 삭제할 수 있게 만들고 앱인토스 안에서 사용감이 괜찮은지 확인한다.
