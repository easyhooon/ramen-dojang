...
외부 검색 후보 보강, 사진, 지도 도장깨기 통계가 들어오면 아래 방향으로 확장한다. 사용자 모델은 MVP 모델에 포함한다. 라멘집 후보(`shop_candidates`)는 외부 검색 API를 바로 `shops`에 넣지 않기 위해 초기 스키마에 포함했고, 사진은 아직 migration에 포함하지 않는다.
