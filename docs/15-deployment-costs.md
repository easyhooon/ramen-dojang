...
1차 배포는 Render 최소 유료 구성이 가장 낫다. 월 $13로 API와 DB를 모두 외부에 열 수 있고, 비용 예측도 쉽다.
...
AWS는 서비스가 AWS를 요구할 때 간다. 예를 들어 사용자 증가, DB 백업/복구, IAM 분리, VPC/private subnet, 장애 대응, 모니터링이 실제 운영 요구가 될 때 전환한다.
