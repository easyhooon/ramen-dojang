...
### 토스 로그인은 늦추되 익명 식별은 준비한다

초기 앱인토스 샌드박스 확인은 서버 catalog와 로컬 개인 기록으로 간다. 다만 방문 기록과 위시리스트를 서버에 저장하는 순간 사용자 소유권이 필요하므로, 최소 식별은 앱인토스 비게임 미니앱의 `getAnonymousKey`를 우선 사용한다. 토스 로그인은 개인정보 조회, 결제 상태 조회, 계정 연결이 필요해질 때 별도 세로 slice로 붙인다.
